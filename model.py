...
class SpatialAppearanceEncoder(nn.Module):
    def __init__(self, ngf=64, blur_kernel=[1, 3, 3, 1], size=256, att_cat=False):
        super().__init__()
        self.size = size
        self.dp_uv_lookup_256_np = np.load('util/dp_uv_lookup_256.npy')
        input_nc = 3 + 3 + 1 # course RGB, warped RGB and sil
        if att_cat:
            input_nc += 1
...
class Generator(nn.Module):
    def __init__(
        ...
        self.appearance_encoder = SpatialAppearanceEncoder(size=size, att_cat=att_cat)
    ...
    def forward(
        ...
        styles = self.appearance_encoder(appearance, sil)
